...
all.ests <- function(fiscal, usa){
  ...
  reg14 <- c(fiscal, usa, "rec_liv_tot_2", "rec_dir_tot_2", "D_dilma", "D_lula", "embi_br") %>% estimation

  reg15 <- c(fiscal, usa, "rec_liv_tot_2", "rec_dir_tot_2", "D_dilma", "D_lula", "hiato", "tx_poup") %>% estimation
  reg16 <- c(fiscal, usa, "rec_liv_tot_2", "rec_dir_tot_2", "D_dilma", "D_lula", "hiato", "ind_termos") %>% estimation 
  reg17 <- c(fiscal, usa, "rec_liv_tot_2", "rec_dir_tot_2", "D_dilma", "D_lula", "hiato", "embi_br") %>% estimation

  reg18 <- c(fiscal, usa, "rec_liv_tot_2", "rec_dir_tot_2", "D_dilma", "D_lula", "ind_termos", "tx_poup") %>% estimation
  reg19 <- c(fiscal, usa, "rec_liv_tot_2", "rec_dir_tot_2", "D_dilma", "D_lula", "embi_br", "tx_poup") %>% estimation
  ...
  est17 <- reg17 %>% adj.fitted; est18 <- reg18 %>% adj.fitted; est19 <- reg19 %>% adj.fitted
  
  coef15 <- reg15 %>% ext.coefs; coef16 <- reg16 %>% ext.coefs
  coef17 <- reg17 %>% ext.coefs; coef18 <- reg18 %>% ext.coefs; coef19 <- reg19 %>% ext.coefs

  ests <- cbind(est11, est12, est13, est14, est15, est16, est17, est18, est19) 
  #est11, est12, est13, est14, 
  coefs <- cbind(coef11, coef12, coef13, coef14, coef15, coef16, coef17, coef18, coef19) %>% apply(MARGIN=1, FUN=mean)
  #coef11, coef12, coef13, coef14,
  
  return(list(ests, coefs))}

df_mm <- df_mm[-c(nrow(df_mm), (nrow(df_mm)-1))]

df_mm$div_brut_1           <- lag(df_mm$div_brut,         -1)
df_mm$result_prim_cons_1   <- lag(df_mm$result_prim_cons, -1)
df_mm$result_prim_ajust_1  <- lag(df_mm$result_prim_ajust,-1)

df_mm$rec_liv_tot_2  <- lag(df_mm$rec_liv_tot,-2)
df_mm$rec_dir_tot_2  <- lag(df_mm$rec_dir_tot,-2)
...
ggplot(plot_data, aes(x=datas)) + geom_line(aes(y=med), linetype="dashed") + 
  geom_line(aes(y=hpfilter(med,freq=4)$trend), color="orange", size=1) + 
