...
source("C:/Users/PIETCON/Documents/Codes/Bases/base_pnad.R", local = TRUE)
...
Y <- ts(pib_dsz$pib, freq=4, start=c(1996,1))
...
Pi <- aggregate(ts(ipca$ipca_nuc_ms, freq=12, start=c(1996,1)),nfrequency=4)
...
y = matrix(log(data), nrow = length(na.omit(C)), ncol = 3)
...
# FILTRO HP
gdp = ts(log(data$Y),start = c(2001,1), end = c(year(as.Date(t[length(t)])),quarter(as.Date(t[length(t)]))), frequency = 4)
...
Yn <- (Yn.lin.trend + Yn.hp + Yn.prod.fn + Yn.areosa + Yn.bevnel)/5

write.csv(cbind(t, Yn), file="Data/EST_prod_pot.csv", row.names = FALSE)
